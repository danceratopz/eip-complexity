...
## Comparison with human and LLM assessments

[`docs/hegota/comparison.html`](docs/hegota/comparison.html) compares the Jev totals with the
reasoning-LLM and human assessments collected by the
[retrospective EIP complexity study](https://danceratopz.github.io/retrospective-eip-complexity/prospective/hegota/?evaluator=llm).
Because that study scored older EIP versions, [`configs/hegota-2026-08-26.toml`](configs/hegota-2026-08-26.toml)
runs Jev on exactly the EIP text it used (results in `docs/hegota-2026-08-26/`), which makes the
Jev-versus-LLM comparison text-identical and also measures how much a month of EIP edits moved
Jev's totals. The LLM and human numbers are vendored with their source commit in
[`docs/hegota/comparison-data.json`](docs/hegota/comparison-data.json). To refresh:

```sh
uv run scripts/extract_comparison_data.py --retrospective ../retrospective-complexity-eval
uv run scripts/compare_hegota.py --eips-repo ../EIPs     # embeds per-EIP diffs when the clone is present
```

The comparison page is generated by hand and committed; the Pages workflow serves it as a static file.
